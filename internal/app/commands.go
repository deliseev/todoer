...
type CreateTaskCommand struct {
	...
	DueDate *time.Time
	// IdempotencyKey — ключ, которым клиент отличает повтор от нового
	// запроса. Пустая строка означает, что защиты от дубля он не просил:
	// требовать ключ ото всех значило бы сломать тех, кто про него не знает.
